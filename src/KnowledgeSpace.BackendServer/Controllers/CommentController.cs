...
namespace KnowledgeSpace.BackendServer.Controllers
{
    public partial class KnowledgeBaseController
    {
        ...
        public async Task<IActionResult> PostComment(int knowledgeBaseId, [FromBody] CommentCreateRequest request)
        {
            ...
            {
                return BadRequest(new ApiResponseBadRequest("Create comment failed"));
            }
        }
        ...
        public async Task<IActionResult> PutComment(int commentId, [FromBody] CommentCreateRequest request)
        {
            var comment = await _context.Comments.FindAsync(commentId);
            if (comment == null)
                return BadRequest(new ApiResponseBadRequest($"Cannot found comment with id: {commentId}"));
            ...
            return BadRequest(new ApiResponseBadRequest($"Update comment failed"));
        }
        ...
        public async Task<IActionResult> DeleteComment(int knowledgeBaseId, int commentId)
        {
            var comment = await _context.Comments.FindAsync(commentId);
            if (comment == null)
                return NotFound(new ApiResponseBadRequest ($"Cannot found the comment with id: {commentId}"));
            ...
            return BadRequest(new ApiResponseBadRequest($"Delete comment failed"));
        }
